...
<div class="grid grid-cols-1 gap-5 md:grid-cols-2">
    ...
    <div class="md:col-span-2">
        ...
        <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $metaActual->titulo ?? '') }}"
            placeholder="Fondo de emergencia" class="metas-input">
    ...
    <div>
        ...
        <input type="number" step="0.01" min="0.01" name="monto_objetivo" id="monto_objetivo"
            value="{{ old('monto_objetivo', $metaActual->monto_objetivo ?? '') }}" placeholder="0.00"
            class="metas-input">
    ...
    <div>
        ...
        <input type="number" step="0.01" min="0" name="monto_actual" id="monto_actual"
            value="{{ old('monto_actual', $metaActual->monto_actual ?? 0) }}" placeholder="0.00" class="metas-input">
    ...
    <div>
        ...
        <select name="prioridad" id="prioridad" class="metas-select">
            <option value="" disabled {{ old('prioridad', $metaActual->prioridad ?? '') === '' ? 'selected' : '' }}>
                Seleccione</option>
            <option value="baja" {{ old('prioridad', $metaActual->prioridad ?? '') === 'baja' ? 'selected' : '' }}>Baja
            </option>
            <option value="media" {{ old('prioridad', $metaActual->prioridad ?? '') === 'media' ? 'selected' : '' }}>
                Media</option>
            <option value="alta" {{ old('prioridad', $metaActual->prioridad ?? '') === 'alta' ? 'selected' : '' }}>Alta
            </option>
        </select>
    ...
    <div>
        ...
        <select name="estado" id="estado" class="metas-select">
            <option value="" disabled {{ old('estado', $metaActual->estado ?? '') === '' ? 'selected' : '' }}>Seleccione
            </option>
            <option value="activa" {{ old('estado', $metaActual->estado ?? '') === 'activa' ? 'selected' : '' }}>Activa
            </option>
            <option value="completada"
                {{ old('estado', $metaActual->estado ?? '') === 'completada' ? 'selected' : '' }}>Completada</option>
            <option value="pausada" {{ old('estado', $metaActual->estado ?? '') === 'pausada' ? 'selected' : '' }}>
                Pausada</option>
        </select>
