<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
    ...
    <div>
        ...
        <input type="text" name="titulo" value="{{ old('titulo', $transaccion->titulo ?? '') }}"
    ...
    <div>
        ...
        <input type="number" step="0.01" name="monto" value="{{ old('monto', $transaccion->monto ?? '') }}"
            placeholder="50.00"
    ...
    <div>
        ...
        <input type="text" name="metodo_pago" value="{{ old('metodo_pago', $transaccion->metodo_pago ?? '') }}"
...
<div class="md:col-span-2">
    ...
    <textarea name="descripcion" rows="3" placeholder="Detalles adicionales sobre la transacción..."
        class="w-full rounded-xl border border-[#26352d] bg-[#111613] px-4 py-3 text-white focus:ring-2 focus:ring-[#72f59a]">{{ old('descripcion', $transaccion->descripcion ?? '') }}</textarea>
...
<div class="col-span-2 rounded-xl border border-[#4a3f1f] bg-[#2a2415] px-4 py-4">
    ...
    <div class="mt-3 flex items-start gap-3 text-sm text-yellow-200">
        ...
        <p class="leading-tight">
            Próximamente se implementará la funcionalidad de cuentas para que puedas asociar esta transacción a una
            cuenta específica.
        </p>
    </div>
